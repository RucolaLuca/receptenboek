<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Het Receptenboek</title>
    <link rel="stylesheet" href="css/style.css">
    <link rel="icon" href="images/Colombia_land_flag.png">
</head>

<body>
    <header>
        <nav>
            <ul>
                <li><a href="index.php">Home</a></li>
                <li><a href="recepten.php">Recipes</a></li>
                <li><a href="specials.php">Specials</a></li>
                <li><a href="https://github.com/RucolaLuca">About Me</a></li>
            </ul>
        </nav>
        <h1 class="header_title"><a href="recepten.php" class="header_title">Colombite</a></h1>
        <p class="header_subtext">Het beste eten uit Colombia.</p>
    </header>
    <main>
        <h2>Wilt u graag eens iets maken uit de Colombiaanse keuken?</h2>
        <h3>Dan bent u hier op de goede plek!</h3>
        <h3>Hier bij Colombite hebben we de lekkerste gerechten voor u!</h3>
    </main>
    <footer>
        <div class="person-list">
            <h3>Maak kennis met het team.</h3>
            <ul>
                <li>Projectleider: Cornelius Arne</li>
                <li>Begeleiding: ICT Academie Nova College Haarlem</li>
                <li>Developer: Luca Wijsman</li>
            </ul>
        </div>
    </footer>
</body>

</html>